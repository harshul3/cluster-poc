...
const logFilePath = "cpu_memory_usage_poc4.log"; // Path to the log file
...
const testCluster = async () => {
  ...
  cluster.task(async ({ page, data }) => {
    const { report, type, filters } = data;
    ...
    try {
      ...
      if (type === 1) {
        ...
        await applyOneFiltersAndGeneratePDF(page, loadedPage, filters);
      }
    } catch (error) {
  ...
  const applyOneFiltersAndGeneratePDF = async (page, loadedPage, filter: any) => {
    console.log("Applying filter:", filter);
    const result = await page.evaluate((f) => setReportFilter(f), [filter]);
    await sleep(10000); // Wait for the filter to be applied
    console.log("Filter applied result:", result);
  ...
  async function createTasks(filters: any[]) {
     const task = filters.map((filter) => ({
          report: report2Details,
          type: 2,
      filters: filter,
    }));
    tasks = [r2detail1, ...task];
  }


  await createTasks(batches[1]);
